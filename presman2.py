...
def runStatementFetch(con, sql_text):
	cursor=con.cursor()
        sql = cursor.execute(sql_text)
...
def resman_sess_io():
	sql_text = 'WITH sessIO as (SELECT current_consumer_group, current_small_read_megabytes+current_large_read_megabytes+current_small_write_megabytes+current_large_write_megabytes sessions_io_total from v$RSRC_SESSION_INFO where current_consumer_group not like \'ORA$%\') select current_consumer_group NAME, sum(current_IO_service_time) IO_SERVICE_TIME, sum(current_small_read_megabytes)  SM_READ_MB, sum(current_large_read_megabytes) LM_READ_MB, sum(current_small_write_megabytes) SM_WRITE_MB, sum(current_large_write_megabytes) LM_WRITE_MB, round(((select SUM(sessions_io_total) from sessIO where current_consumer_group = a.current_consumer_group) / (select SUM(sessions_io_total) from sessIO))*100, 2) TOTAL_IO_PCT, SUM(current_undo_consumption) UNDO_CONSUMPTION FROM v$RSRC_SESSION_INFO a where current_consumer_group not like \'ORA$%\' group by current_consumer_group'	
	return sql_text

def resman_cpu_total():
	sql_text = 'SELECT SUM(consumed_cpu_time) consumed_cpu_percent from v$rsrc_consumer_group where name not like \'ORA$%\''
	return sql_text
...
def resman_cpu_calculate(con):
	...
	print x
	print '--> CPU Consumed by Consumer Group Graph\n'
...
def resman_sess_io_calculate(con):
	...
	print x
	print '--> Session I/O Consumed by Consumer Group Graph\n'
